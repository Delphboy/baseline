...
class DataLoader(data.Dataset):
    ...
    def get_batch(self, split, batch_size=None, seq_per_img=None):
        ...
        for i in range(batch_size):
            # fetch image
            tmp_fc, tmp_att,slot_items,ix, tmp_wrapped = self._prefetch_process[split].get()
            slots_batch.append(slot_items)
    ...
    def __getitem__(self, index):
        ...
        return (np.load(os.path.join(self.input_fc_dir, str(self.info['images'][index]['id']) + '.npy')),
                att_feat,
                slots,
                index)
